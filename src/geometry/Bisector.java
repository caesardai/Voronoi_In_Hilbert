...
//	}
	...
	/*
	 * Compute discriminant for the intersection point between the bisector and a
	 * line; this corresponds for solution of x private Double
	 * computeDiscriminantX(Double[] line) { Double a = line[0]; Double b = line[1];
	 * Double c = line[2];
	 * 
	 * // System.out.println("line: " + Util.printLineEq(new Double[] {a, b, c}));
	 * Double first = Math.pow(- this.C * c * b + 2 * this.B * a * c + this.D *
	 * Math.pow(b, 2) - this.E * a * b, 2); Double second = this.A * Math.pow(b, 2)
	 * - this.C * a * b + this.B * Math.pow(a, 2); Double third = this.B *
	 * Math.pow(c, 2) - this.E * c * b + this.F * Math.pow(b, 2);
	 * 
	 * return first - 4 * second * third; }
	 */
	...
	/*
	 * Compute discriminant for the intersection point between the bisector and a
	 * line; this corresponds for solution of y private Double
	 * computeDiscriminantY(Double[] line) { Double a = line[0]; Double b = line[1];
	 * Double c = line[2];
	 * 
	 * Double first = Math.pow(2 * this.A * b * c - this.C * c * a - this.D * b * a
	 * + this.E * Math.pow(a, 2), 2); Double second = this.A * Math.pow(b, 2) -
	 * this.C * b * a + this.B * Math.pow(a, 2); Double third = this.A * Math.pow(c,
	 * 2) - this.D * c * a + this.F * Math.pow(a, 2);
	 * 
	 * return first - 4 * second * third; }
	 */
	...
	/*
	 * Compute the x-values of any intersection points between a given line and the
	 * bisector private Double[] intersectionBisectorLineX(Double[] line) { Double[]
	 * solutions = new Double[2]; Double a = line[0]; Double b = line[1]; Double c =
	 * line[2];
	 * 
	 * // Double firstTerm = -(2 * this.B * a * c + this.D * Math.pow(b, 2) - this.E
	 * * a * b - this.C * c * b); Double firstTerm = -(-this.C * b * c + 2 * this.B
	 * * a * c + this.D * Math.pow(b, 2) - this.E * a * b); Double discriminant =
	 * this.computeDiscriminantX(line); Double denominator = 2 * (this.A *
	 * Math.pow(b, 2) - this.C * a * b + this.B * Math.pow(a, 2));
	 * 
	 * solutions[0] = (firstTerm + Math.sqrt(discriminant)) / denominator;
	 * solutions[1] = (firstTerm - Math.sqrt(discriminant)) / denominator;
	 * 
